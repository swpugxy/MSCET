...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.proj_3x3 = nn.Conv2d(in_chans, embed_dim//2, kernel_size=3, stride=patch_size, padding=1)
        self.proj_7x7 = nn.Conv2d(in_chans, embed_dim//2, kernel_size=7, stride=patch_size, padding=3)
    ...
    def forward(self, x):
        ...
        x_3x3 = self.proj_3x3(x)
        x_7x7 = self.proj_7x7(x)
        x = torch.cat((x_3x3,x_7x7), dim=1)
...
class MSCET(nn.Module):
    def __init__(self, pretrained_cfg=None, img_size=224, in_chans=3, num_classes=1000, embed_dims=[46, 92, 184, 368],
        ...
        # 定义3x3和5x5卷积，假设最终的通道数由两者的和组成
        self.conv3x3 = nn.Conv2d(3, stem_channel//2, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn3x3 = nn.BatchNorm2d(stem_channel//2)
        self.relu3x3 = nn.GELU()

        # self.conv5x5 = nn.Conv2d(3, 10, kernel_size=5, stride=2, padding=2, bias=False)
        # self.bn5x5 = nn.BatchNorm2d(10)
        # self.relu5x5 = nn.GELU()

        self.conv7x7 = nn.Conv2d(3, stem_channel//2, kernel_size=7, stride=2, padding=3, bias=False)
        self.bn7x7 = nn.BatchNorm2d(stem_channel//2)
        self.relu7x7 = nn.GELU()

        # 1x1卷积用于合并特征和调整通道数
        # self.conv1x1 = nn.Conv2d(stem_channel, stem_channel, kernel_size=1, bias=False,groups=2)
        # self.bn1x1 = nn.BatchNorm2d(stem_channel)
        # self.relu1x1 = nn.GELU()
        self.se_a = Se(64,16)
        self.se_b = Se(128, 16)
        self.se_c = Se(256, 16)
        self.se_d = Se(512, 16)
...
def _create_mscet_model(pretrained=False, distilled=False, **kwargs):
    default_cfg = _cfg()
    ...
    model = MSCET(img_size=img_size, num_classes=num_classes, representation_size=repr_size, **kwargs)
...
@register_model
def mscet(pretrained=False, **kwargs):
    model_kwargs = dict(
    ...
    model = _create_mscet_model(pretrained=pretrained, **model_kwargs)
    return model
